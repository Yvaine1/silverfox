...
__attribute__((unused)) volatile u32 RecvFlagA53;
...
void ipi_a53_2_r50_init()
{
    IPI_Init(&Ipi_a53,IPI_ID_CH1,(FMSH_InterruptHandler)Ipi_Irq_Handler_A53);
}

void ipi_demo()
{
    IPI_Init(&Ipi_a53,IPI_ID_CH1,(FMSH_InterruptHandler)Ipi_Irq_Handler_A53);
    IPI_Init(&Ipi_cr5_1,IPI_ID_CH9,(FMSH_InterruptHandler)Ipi_Irq_Handler_CR5_1);
    SetMPURegion(SHMEM_ADDR_A53_TO_CR5_0,TOTAL_SHMEM_SIZE,7,NORM_SHARED_NCACHE|PRIV_RW_USER_RW);
...
void prvRecvMessage (void *pvParameters)
{
    u32 recv_a53[20];
    u32 recv_cr5_1[20];
    ...
    for (;;)
    {
        if (RecvFlagA53)
        {
            ...
            fmsh_print("rpu0:recv a53:\n\r");
            dump_message(recv_a53,20);
            
            shmem_msg_construct(send_a53,20,SHMEM_ADDR_CR5_0_TO_A53);
            Send_Message(&Ipi_a53,send_a53,20,SHMEM_ADDR_CR5_0_TO_A53,REMOTE_MASK_CH7);
        }
        
        if (RecvFlagCR5_1)
        {
            RecvFlagCR5_1 = 0;
            Recv_Message(recv_cr5_1,20,SHMEM_ADDR_CR5_1_TO_0);
            fmsh_print("rpu0:recv rpu1:\n\r");
            dump_message(recv_cr5_1,20);
            shmem_msg_construct(send_cr5_1,20,SHMEM_ADDR_CR5_0_TO_1);
            Send_Message(&Ipi_cr5_1,send_cr5_1,20,SHMEM_ADDR_CR5_0_TO_1,REMOTE_MASK_CH10);
        }
    }
}
