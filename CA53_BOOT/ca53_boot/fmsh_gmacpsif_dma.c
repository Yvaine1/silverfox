...
struct ftopology_t ftopology[] = {
    {
        GMAC_SELECT_BASEADDR,
        fgmac_type_gmacps,
        0x0,
        0x0,
        0xF8F00100,
        GMAC_SELECT_INTR,  //FPAR_GMACPS_1_INTR,
    },
};
 ...
 *********************************************************************************/
 ...
 __attribute__ ((section(".GMAC_DESC"), aligned (0x200000)))
u8_t bd_space[0x200000];
#else
...
void setup_rx_bds(fgmacpsif_s *xgmacpsif, FGmacPs_BdRing *rxring)
{
    ...
    while (freebds > 0) {
        ...
        // p = (struct pbuf*)mem_malloc(FGMACPS_MAX_FRAME_SIZE);
#endif
        if (!p) {
#if LINK_STATS
