...
{
	fmsh_print("UDP client connecting to %s on port %d\r\n",
			UDP_SERVER_IP_ADDRESS, UDP_CONN_PORT);
	fmsh_print("On Host: Run $iperf -s -i %d -u\r\n\r\n",
			INTERIM_REPORT_INTERVAL);
}

static void print_udp_conn_stats(struct netif *netif)
{
	fmsh_print("[%3d] local %s port %d connected with ",
			client.client_id, inet_ntoa(netif->ip_addr),
			pcb[0]->local_port);
	fmsh_print("%s port %d\r\n",inet_ntoa(pcb[0]->remote_ip),
			pcb[0]->remote_port);
	fmsh_print("[ ID] Interval\t\tTransfer   Bandwidth\n\r");
}
		...
		double data, enum measure_t type)
{
	...
	sprintf(outString, format, data, kLabel_UDP[conv]);
}
		...
		enum report_type report_type)
{
	...
	/* On 32-bit platforms, fmsh_print is not able to print
	...
	fmsh_print("[%3d] %s  %sBytes  %sbits/sec\n\r", client.client_id,
			time, data, perf);

	if (report_type == INTER_REPORT)
		client.i_report.last_report_time += duration;
	else
		fmsh_print("[%3d] sent %llu datagrams\n\r",
				client.client_id, client.cnt_datagrams);
}


static void reset_stats(struct netif *netif)
{
	client.client_id++;
	/* Print connection statistics */
	print_udp_conn_stats(netif);
...
static void udp_packet_send(u8_t finished)
{
	...
	for (i = 0; i < NUM_OF_PARALLEL_CLIENTS; i++) {
		...
		if (!packet) {
			fmsh_print("error allocating pbuf to send\r\n");
			return;
		} else {
		...
		while (retries) {
			...
			if (err != ERR_OK) {
				fmsh_print("Error on udp_send: %d\r\n", err);
				retries--;
				delay_us(100);
			} else {
		...
		if (!retries) {
			...
			fmsh_print("Too many udp_send() retries, ");
			fmsh_print("Terminating application\n\r");
			udp_conn_report(diff_ms, UDP_DONE_CLIENT);
			fmsh_print("UDP test failed\n\r");
			udp_remove(pcb[i]);
			pcb[i] = NULL;
		}
		if (finished == FINISH)
		...
		/* For  SGMII, At high speed,
		 * "pack dropped, no space" issue observed.
		 * To avoid this, added delay_us of 2us between each
		 * packets.
		 */
	...
	if (END_TIME || REPORT_INTERVAL_TIME) {
		...
		if (END_TIME) {
			...
			if (diff_ms >= END_TIME) {
				...
				udp_conn_report(diff_ms, UDP_DONE_CLIENT);
				fmsh_print("UDP test passed Successfully\n\r");
				return;
			}
		}
	}
...
void start_application_udp_perf_client(struct netif *netif)
{
	err_t err;
	...
	if (!err) {
		fmsh_print("Invalid Server IP address: %d\r\n", err);
		return;
	}
	...
	for (i = 0; i < NUM_OF_PARALLEL_CLIENTS; i++) {
		...
		pcb[i] = udp_new();
		if (!pcb[i]) {
			fmsh_print("Error in PCB creation. out of memory\r\n");
			return;
		}

		err = udp_connect(pcb[i], &remote_addr, UDP_CONN_PORT);
		if (err != ERR_OK) {
			fmsh_print("udp_client: Error on udp_connect: %d\r\n", err);
			udp_remove(pcb[i]);
			return;
		}
	}
