...
void print_app_header_tcp_perf_client()
{
        fmsh_print("TCP client connecting to %s on port %d\r\n",
			TCP_SERVER_IP_ADDRESS, TCP_CONN_PORT);
	fmsh_print("On Host: Run $iperf -s -i %d -w 2M\r\n",
		...
		double data, enum measure_t type)
{
	...
	sprintf(outString, format, data, kLabel_TCP[conv]);
}
		...
		enum report_type report_type)
{
	...
	fmsh_print("[%3d] %s  %sBytes", client.client_id,
			time, data);
        fmsh_print("%sbits/sec\n\r", perf);
...
static void tcp_client_err(void *arg, err_t err)
{
	...
	fmsh_print("TCP connection aborted\n\r");
}
	...
	if (client.end_time || client.i_report.report_interval_time) {
		...
		if (client.end_time) {
			...
			if (diff_ms >= client.end_time) {
				...
				fmsh_print("TCP test passed Successfully\n\r");
				tcp_client_close(c_pcb);
				c_pcb = NULL;
				return ERR_OK;
			}
...
void transfer_data_tcp_perf_client(void)
{
	if (client.client_id)
		tcp_send_perf_traffic();
}

void start_application_tcp_perf_client(void)
{
	err_t err;
	struct tcp_pcb* pcb;
	...
	if (err) {
		fmsh_print("Error on tcp_connect: %d\r\n", err);
