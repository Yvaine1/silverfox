 ...
 */
extern void CPS_WritePhysAddress32(volatile uint32_t* location,
                                   uint32_t addrValue);
 ...
 */
extern void CPS_BufferCopy(volatile uint8_t* dst, volatile const uint8_t* src,
                           uint32_t size);
 ...
 * @param[in] address Virtual address of memory region. (If an MMU is not in
 * use, this will be equivalent to the physical address.) This address should be
 ...
 * @param[in] devInfo   This parameter can be used to pass implementation
 * specific data to this function.  The content and use of this parameter is up
 * to the implementor of this function to determine, and if not required it may
 * be ignored. For example, under Linux it can be used to pass a pointer to the
 * device struct to be used in a call to dma_sync_single_for_device().  If used,
 * the parameter should be passed to the core driver at initialisation as part
 * of the configurationInfo struct.  Please see the core driver documentation
 ...
 * @param[in] delay number of microseconds to delay software execution
 */
extern void CPS_DelayUs(uint32_t delay);
