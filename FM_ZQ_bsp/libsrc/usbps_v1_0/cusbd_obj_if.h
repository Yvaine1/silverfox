...
typedef struct CUSBD_OBJ_s {
     ...
     */
    uint32_t (*probe)(const CUSBD_Config* config);
     ...
     * @return CDN_EINVAL if illegal/inconsistent values in 'config'
     */
    uint32_t (*stop)(const CUSBD_PrivateData* pD);
     ...
     */
    void (*epFifoFlush)(const CUSBD_PrivateData* pD, const CUSBD_Ep* ep);
     ...
     * @return CDN_EINVAL if pD or ep is NULL
     */
    uint32_t (*reqDequeue)(const CUSBD_PrivateData* pD, const CUSBD_Ep* ep,
                           const CUSBD_Req* req);
