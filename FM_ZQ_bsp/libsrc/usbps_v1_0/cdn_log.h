 ...
 *
 * There is also possibility to provide own implementation of printf() routine
 * by defining function: DbgPrint that provides name of functions that replace
 * printf(). Their prototype: void DbgPrint(const char *fmt, ...)
 *
 * If DEBUG symbol is defined, then appropriate prototype is declared within
 * this header file.
 *
 * If DEBUG is not defined, print is not used. Existing macros will not print
 * any data.
 */
...
#define cDbgMsg(_t, _x, ...)                                               \
    (((_x) == 0) || (((_t) & g_dbg_enable_log) && ((_x) <= g_dbg_log_lvl)) \
         ? printf(__VA_ARGS__)                                             \
         : 0)
...
#define vDbgMsg(log_lvl, module, msg, ...)                              \
    DbgMsg((log_lvl), (module), (DEBUG_PREFIX msg), __func__, __LINE__, \
           g_dbg_log_cnt++, __VA_ARGS__)

// ******** Default cvDbgMsg ********
#define cvDbgMsg(log_lvl, module, msg, ...)                              \
    cDbgMsg((log_lvl), (module), (DEBUG_PREFIX msg), __func__, __LINE__, \
            g_dbg_log_cnt++, __VA_ARGS__)
