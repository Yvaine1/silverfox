 ...
 * Defines
 **********************************************************************/
/** Size of EP0 status response buffer in bytes */
#define CUSBD_EP0_RESP_BUFF_SZ 64U
...
typedef struct CUSBD_ListHead_s CUSBD_ListHead;
...
typedef void (*CUSBD_CbEpFifoFlush)(const CUSBD_PrivateData* pD,
                                    const CUSBD_Ep* ep);
 ...
 */
typedef uint32_t (*CUSBD_CbReqDequeue)(const CUSBD_PrivateData* pD,
                                       const CUSBD_Ep* ep,
                                       const CUSBD_Req* req);
...
typedef void (*CUSBD_CbConfigured)(CUSBD_PrivateData* pD,
                                   uint32_t enabledEpMask);
