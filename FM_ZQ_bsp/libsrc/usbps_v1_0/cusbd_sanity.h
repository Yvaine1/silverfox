...
static inline uint32_t CUSBD_CallbacksSF(const CUSBD_Callbacks* obj);
static inline uint32_t CUSBD_ConfigSF(const CUSBD_Config* obj);
static inline uint32_t CUSBD_DevSF(const CUSBD_Dev* obj);
static inline uint32_t CUSBD_EpPrivateSF(const CUSBD_EpPrivate* obj);
static inline uint32_t CUSBD_EpSF(const CUSBD_Ep* obj);
static inline uint32_t CUSBD_PrivateDataSF(const CUSBD_PrivateData* obj);
static inline uint32_t CUSBD_ReqSF(const CUSBD_Req* obj);

static inline uint32_t CUSBD_SanityFunction1(const CUSBD_Config* config);
static inline uint32_t CUSBD_SanityFunction2(const CUSBD_PrivateData* pD,
                                             const CUSBD_Config* config,
                                             const CUSBD_Callbacks* callbacks);
static inline uint32_t CUSBD_SanityFunction3(const CUSBD_PrivateData* pD);
static inline uint32_t CUSBD_SanityFunction7(const CUSBD_PrivateData* pD,
                                             const CUSBD_Ep* ep,
                                             const uint8_t* desc);
static inline uint32_t CUSBD_SanityFunction8(const CUSBD_PrivateData* pD,
                                             const CUSBD_Ep* ep);
static inline uint32_t CUSBD_SanityFunction13(const CUSBD_PrivateData* pD,
                                              const CUSBD_Ep* ep,
                                              const CUSBD_Req* req);
static inline uint32_t CUSBD_SanityFunction15(const CUSBD_PrivateData* pD,
                                              const CUSBD_Dev** dev);
static inline uint32_t CUSBD_SanityFunction16(const CUSBD_PrivateData* pD,
                                              const uint32_t* numOfFrame);
static inline uint32_t CUSBD_SanityFunction19(
    const CUSBD_PrivateData* pD, const CH9_ConfigParams* configParams);
...
static inline uint32_t CUSBD_PrivateDataSF (const CUSBD_PrivateData* obj)
{
    ...
    {
        if (USBSSP_DriverResourcesTSF(&obj->xhciDriverResources) == CDN_EINVAL)
        {
            ret = CDN_EINVAL;
        }
        ...
        if ((obj->ep0NextState != USBSSP_EP0_UNCONNECTED) &&
            (obj->ep0NextState != USBSSP_EP0_HALT_PENDING) &&
            (obj->ep0NextState != USBSSP_EP0_HALT_SETUP_PENDING) &&
            (obj->ep0NextState != USBSSP_EP0_HALTED) &&
            (obj->ep0NextState != USBSSP_EP0_SETUP_PENDING) &&
            (obj->ep0NextState != USBSSP_EP0_SETUP_PHASE) &&
            (obj->ep0NextState != USBSSP_EP0_DATA_PHASE) &&
            (obj->ep0NextState != USBSSP_EP0_STATUS_PHASE))
        {
            ret = CDN_EINVAL;
        }
        ...
        for (idx_ep_in_container = 0; idx_ep_in_container < 16U;
             idx_ep_in_container++)
        // for (idx_ep_in_container = 0; idx_ep_in_container < 1U;
        // idx_ep_in_container++)
        {
        ...
        // for (idx_ep_out_container = 0; idx_ep_out_container < 1U;
        // idx_ep_out_container++)
        {
            if (CUSBD_EpPrivateSF(
                    &obj->ep_out_container[idx_ep_out_container]) == CDN_EINVAL)
            {
                ret = CDN_EINVAL;
            }
        }
...
static inline uint32_t CUSBD_SanityFunction1 (const CUSBD_Config* config)
{
    ...
    if (CUSBD_ConfigSF(config) == CDN_EINVAL)
    {
        ret = CDN_EINVAL;
    }
    else
