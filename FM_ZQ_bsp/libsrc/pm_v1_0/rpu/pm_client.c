...
static struct FPm_Master pm_rpu_0_master = {
	.node_id = NODE_RPU_0,
...
static struct FPm_Master pm_rpu_1_master = {
	.node_id = NODE_RPU_1,
...
const enum FPmNodeId subsystem_node = NODE_RPU;
/* By default, lock-step mode is assumed */
struct FPm_Master *primary_master = &pm_rpu_0_master;
...
void FPm_ClientSuspendFinalize(void)
{
	...
	ctrlReg = mfcp(CP15_SYS_CONTROL);
#elif defined (__ICCARM__)
	mfcp(CP15_SYS_CONTROL, ctrlReg);
#endif
	if ((CP15_CONTROL_C_BIT & ctrlReg) != 0U) {
        Fmsh_DCacheFlush();
	}
...
/**
 * FPm_ClientSetPrimaryMaster() -Set primary master
 *
...
#if defined (__GNUC__)
	master_id = mfcp(CP15_MULTI_PROC_AFFINITY) & PM_CLIENT_AFL0_MASK;
#elif defined (__ICCARM__)
	mfcp(CP15_MULTI_PROC_AFFINITY, master_id);
