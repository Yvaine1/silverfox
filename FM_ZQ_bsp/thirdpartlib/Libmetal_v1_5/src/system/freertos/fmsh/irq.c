...
/*
 * @file	freertos/fmsh/irq.c
 * @brief	freertos libmetal FMSH irq controller definitions.
 */
...
#include <metal/sys.h>
#include <metal/system/freertos/fmsh/sys.h>

#define MAX_IRQS FMSH_MAXIRQS

static struct metal_irq irqs[MAX_IRQS]; /**< Linux IRQs array */

static void metal_fmsh_irq_set_enable(struct metal_irq_controller *irq_cntr,
...
/**< FMSH common platform IRQ controller */
static METAL_IRQ_CONTROLLER_DECLARE(fmsh_irq_cntr,
				    0, MAX_IRQS,
				    NULL,
				    metal_fmsh_irq_set_enable, NULL,
				    irqs);
...
int metal_fmsh_irq_init(void)
{
	...
	ret =  metal_irq_register_controller(&fmsh_irq_cntr);
	if (ret < 0) {
