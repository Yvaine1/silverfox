...
void XUartNs550_SendByte(UINTPTR BaseAddress, u8 Data)
{
	 ...
	 */
	FMSH_WriteReg(BaseAddress, XUN_THR_OFFSET, (u32)Data);
}
...
u8 XUartNs550_RecvByte(UINTPTR BaseAddress)
{
	 ...
	 */
	return (u8) FMSH_ReadReg(BaseAddress, XUN_RBR_OFFSET);
}
...
void XUartNs550_SetBaud(UINTPTR BaseAddress, u32 InputClockHz, u32 BaudRate)
{
	 ...
	 * NS16550D spec sheet
	 */
	FMSH_WriteReg(BaseAddress, XUN_DLL_OFFSET, 0xFF);
	FMSH_WriteReg(BaseAddress, XUN_DLM_OFFSET, BaudMSB);
	FMSH_WriteReg(BaseAddress, XUN_DLL_OFFSET, BaudLSB);
