...
u16 XUartNs550_GetOptions(XUartNs550 *InstancePtr)
{
	...
	for (Index = 0; Index < XUN_NUM_OPTIONS; Index++) {
		...
		if (OptionsTable[Index].RegisterOffset == XUN_FCR_OFFSET) {
			...
			Register = FMSH_ReadReg(InstancePtr->BaseAddress,
...
*
* @return
*		- FMSH_SUCCESS if the trigger level was set
...
int XUartNs550_SetFifoThreshold(XUartNs550 *InstancePtr, u8 TriggerLevel)
{
	 ...
	 */
	FMSH_ASSERT(InstancePtr != NULL);
	FMSH_ASSERT((TriggerLevel == XUN_FIFO_TRIGGER_14) ||
...
u8 XUartNs550_GetModemStatus(XUartNs550 *InstancePtr)
{
	 ...
	 */
	ModemStatusRegister = FMSH_ReadReg(InstancePtr->BaseAddress,
					XUN_MSR_OFFSET);
...
static u32 ReadFcrRegister(UINTPTR BaseAddress)
{
	 ...
	 */
	IerRegister = FMSH_ReadReg(BaseAddress, XUN_IER_OFFSET);
	FMSH_WriteReg(BaseAddress, XUN_IER_OFFSET, 0);
	 ...
	 */
	FcrRegister = FMSH_ReadReg(BaseAddress, XUN_FCR_OFFSET);
	 ...
	 */
	FMSH_WriteReg(BaseAddress, XUN_IER_OFFSET, IerRegister);
