...
int XUartNs550_CfgInitialize(XUartNs550 *InstancePtr,
					...
					UINTPTR EffectiveAddr, u32 BaudRate)
{
	int Status;
	 ...
	 * level
	 */
	FMSH_WriteReg(InstancePtr->BaseAddress,
				XUN_FCR_OFFSET, XUN_FIFO_ENABLE);
	FMSH_WriteReg(InstancePtr->BaseAddress, XUN_FCR_OFFSET,
					...
					unsigned int NumBytes)
{
	 ...
	 */
	FMSH_ASSERT(InstancePtr != NULL);
	FMSH_ASSERT(BufferPtr != NULL);
	FMSH_ASSERT(InstancePtr->IsReady == COMPONENT_IS_READY);
	FMSH_ASSERT(((signed)NumBytes) >= 0);
	 ...
	 */
	IerRegister = FMSH_ReadReg(InstancePtr->BaseAddress,
						XUN_IER_OFFSET);
	FMSH_WriteReg(InstancePtr->BaseAddress, XUN_IER_OFFSET,
				...
				unsigned int NumBytes)
{
	...
	IerRegister = FMSH_ReadReg(InstancePtr->BaseAddress,
						XUN_IER_OFFSET);
	FMSH_WriteReg(InstancePtr->BaseAddress, XUN_IER_OFFSET, 0);
...
unsigned int XUartNs550_SendBuffer(XUartNs550 *InstancePtr)
{
	...
	if (LsrRegister & XUN_LSR_TX_BUFFER_EMPTY) {
		 ...
		 * are enabled
		 */
		IirRegister = FMSH_ReadReg(InstancePtr->BaseAddress,
							XUN_IIR_OFFSET);
...
unsigned int XUartNs550_ReceiveBuffer(XUartNs550 *InstancePtr)
{
	...
	while (ReceivedCount < InstancePtr->ReceiveBuffer.RemainingBytes) {
		...
		if (LsrRegister & XUN_LSR_BREAK_INT) {
			(void)FMSH_ReadReg(InstancePtr->BaseAddress,
							XUN_RBR_OFFSET);
		 ...
		 */
			...
			FMSH_ReadReg(InstancePtr->BaseAddress,
						XUN_RBR_OFFSET);
...
*
* @return
*		- FMSH_SUCCESS if everything configures as expected
...
int XUartNs550_SetBaudRate(XUartNs550 *InstancePtr, u32 BaudRate)
{
	 ...
	 */
	FMSH_WriteReg(InstancePtr->BaseAddress, XUN_DLL_OFFSET, 0xFF);
	FMSH_WriteReg(InstancePtr->BaseAddress, XUN_DLM_OFFSET,
				BaudMSB);
	FMSH_WriteReg(InstancePtr->BaseAddress, XUN_DLL_OFFSET,
				BaudLSB);
