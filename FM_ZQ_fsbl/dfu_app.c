...
#define USB_BASEADDR            (0xFE200000U)
...
void init_dfu_config (CUSBD_PrivateData *pD)
{
    ...
    pD->config.xhciConfig.hostRegs = USB_BASEADDR + 0x8000U;
    pD->config.xhciConfig.deviceRegs = USB_BASEADDR + 0x4000U;
}

void usb_controller_release (void)
{
    u32 reg = 0U;
    u32 mask = 0U;

    FMSH_WriteReg(USB_SLCR, 0, 0x00112);
    reg = FMSH_ReadReg(USB_CRL_APB, USB_SOF_REF_CLK);
    reg |= USB_SOF_CLKACT;
    FMSH_WriteReg(USB_CRL_APB, USB_SOF_REF_CLK, reg);
    // bus clk
    reg = FMSH_ReadReg(USB_CRL_APB, USB_BUS_REF_CTRL);
    reg |= USB_BUS_CLKACT;
    FMSH_WriteReg(USB_CRL_APB, USB_BUS_REF_CTRL, reg);

    // apb power brige
    reg = FMSH_ReadReg(USB_CRL_APB, RST_LPD_TOP);
    mask = USB_APB_RESET | USB_PWRUP_RESET | USB_BRIDGE_RESET;
    reg |= mask;
    FMSH_WriteReg(USB_CRL_APB, RST_LPD_TOP, reg);
    // apb brige
    reg = FMSH_ReadReg(USB_CRL_APB, RST_LPD_TOP);
    mask = USB_APB_RESET | USB_BRIDGE_RESET;
    reg &= ~mask;
    FMSH_WriteReg(USB_CRL_APB, RST_LPD_TOP, reg);
    //FMSH_WriteReg(0xfe200000, 0x3c, (1U << 25) | (1U << 24));
    //delay_ms(1);
...
u32 FmshFsbl_InitDfu (u32 DeviceFlags)
{
    u32 status=FMSH_SUCCESS;
    
    usb_controller_release();
    delay_ms(1);
    FMSH_WriteReg(0xfe200000, 0x3c, (1U << 25) | (1U << 24));
    DfuVirtFlash = (u8 *)FSBL_DDR_TEMP_BUFFER_ADDRESS;
    ...
    if(status != FMSH_SUCCESS){
        ...
        if(status != FMSH_SUCCESS){
          ...
          if(status != FMSH_SUCCESS){
            ...
            while ((DownloadDone < FSBL_DOWNLOAD_COMPLETE) &&
                   (DfuObj.CurrStatus != STATE_DFU_ERROR))
            {
            }

            if(DownloadDone == FSBL_DOWNLOAD_COMPLETE) {
                         return FMSH_SUCCESS ;
            }
            else
            {
...
{
    (void)memcpy((u8 *)DestAddress, (u8 *)(SrcAddress + FSBL_DDR_TEMP_BUFFER_ADDRESS), Length);
